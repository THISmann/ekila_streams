...
from jazzmin.compat import NoReverseMatch
...
def order_with_respect_to(
    original: List, reference: List, getter: Callable = lambda x: x
) -> List:
    """
...
def get_admin_url(
    instance: Any, admin_site: str = "admin", from_app: bool = False, **kwargs: str
) -> str:
    """
    ...
    try:
        ...
        if not from_app:
            logger.warning(
                gettext(
                    "Could not reverse url from {instance}".format(instance=instance)
                )
            )
...
def get_view_permissions(user: AbstractUser) -> Set[str]:
    ...
    for perm in perms:
        ...
        lower_perms.append(
            "{app}.{perm_codename}".format(app=app, perm_codename=perm_codename.lower())
        )
