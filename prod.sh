...
cp ekila_streams/settings_prod_example.py ekila_streams/settings_prod.py
...
if ! PGPASSWORD=${PGPASS}  psql -U postgres -h ${POSTGRES_HOST} -p ${POSTGRES_PORT} -lqt | cut -d \| -f 1 | cut -d ' ' -f 2 | grep -q "^ekila_db$"; then
    PGPASSWORD=${PGPASS} createdb -U postgres -h ${POSTGRES_HOST} -p ${POSTGRES_PORT} ekila_db
else
...
python manage.py shell -c "$USER_EXISTS" && python manage.py createsuperuser --username radioadm --email radioadm@gmail.com --noinput
